jQuery(document).ready(function ($) {
	...
	$('.header_social_sortable_control').each(function () {
		...
		if (firstData) {
			$('.container-dropdown-header-social .box-dropdown ul li#' + firstData).css('display', 'none');
			$('.container-dropdown-header-social .box-dropdown ul li#' + firstData).attr('status', 'hidden');
		}
		...
		jQuery(window).on("load", function(){
			if (repeatItems >= 0) {
				...
				for (i = 0; i < repeatItems; ++i) {
					// const testing = $('.sortable_repeater_header_social').find('.repeater:eq('+index+')');
					...
					const sosmed = $.trim($('.sortable_repeater_header_social').find('.repeater.'+mylabel+' .box-info .text').text());

					if ('facebook' === mylabel) {

						let getcolor = $('li#customize-control-header_social_icon_color_'+mylabel+'_setting .components-dropdown:eq(1) button');
						...
						if (color) {
							...
							if ('facebook' === sosmed) {
								$('.sortable_repeater_header_social').find('.repeater.'+mylabel+' .box-info .text').next().find('.gradient').css('background', color)
							}
						}

						if ('facebook' === sosmed) {
							$('.sortable_repeater_header_social').find('.repeater.'+mylabel+' .box-info .text').next().next().find('.gradient').css('background', colorOri);
						}
		...
		if (repeatItems > 0) {
			...
			if (repeatItems > 1) {
				...
				for (i = 1; i < repeatItems; ++i) {
					...
					$('.container-dropdown-header-social .box-dropdown ul li#' + dataActive).css('display', 'none');
					$('.container-dropdown-header-social .box-dropdown ul li#' + dataActive).attr('status', 'hidden');
				}
			}

			let checked = true;
			var i;
			const lengthList = $('.container-dropdown-header-social .box-dropdown ul li').length - 1;
			let total = []; 
			for (i = 0; i <= lengthList; ++i) {
				const dataStatus = $('.container-dropdown-header-social .box-dropdown ul li:eq(' + i + ')').attr('status');
				const dataText = $('.container-dropdown-header-social .box-dropdown ul li:eq(' + i + ')').text().replace(/\s/g, '');
				...
				if (checked) {
					if (dataStatus !== 'hidden') {
						...
						// $('.container-dropdown-header-social .box-dropdown .title-active').text(dataText);
						$('.container-dropdown-header-social .box-dropdown .title-active').text(mytext);
						checked = false;
					}
				}
			}
			...
			if (total.length === 7) {
				$('.container-dropdown-header-social').css('display', 'none');
				$('.header_social_sortable_control .add-new-label').css('display', 'none');
			}
		}
	});

	// Make our Repeater fields sortable (Drag and Drop)
	$(this).find('.sortable_repeater_header_social.sortable').sortable({
	...
	// Open toggle slide
	$('.sortable_repeater_header_social.sortable').on('click', '.repeater .box-info .toggle', function (event) {
		$(this).parent().toggleClass('active');
	});

	// Add new item
	$('.sortable_repeater_header_social.sortable').on('click', '.repeater .box-info .close', function (event) {

		const infoSosmed = $(this).parent().parent().find('.box-input-sosmed .sosmed').val().replace(/\s/g, '');
		const activeList = $('.container-dropdown-header-social .box-dropdown ul li[status="show"]').length;

		if (infoSosmed) {
			event.preventDefault();

			// Add dropdown
			$('.container-dropdown-header-social .box-dropdown ul li#' + infoSosmed).attr('status', 'show');
			$('.container-dropdown-header-social .box-dropdown ul li#' + infoSosmed).css('display', 'block');

			// Show dropdown
			if (activeList === 0) {
				const mytext = 'Select item';
				// $('.container-dropdown-header-social .box-dropdown .title-active').text(infoSosmed);
				$('.container-dropdown-header-social .box-dropdown .title-active').text(mytext);
				$('.container-dropdown-header-social').css('display', 'flex');
				$('.header_social_sortable_control .add-new-label').css('display', 'block');
	...
	$('.sortable_repeater_header_social.sortable').change(function () {
		solaceGetAllInputs($(this).parent());
	})

	// Trigger Change with input add new
	$('.sortable_repeater_header_social.sortable').keyup(function () {
		const url = $(this);
		const val = url.val();
	...
	// -------------------------- Dropdown --------------------------
	$(function () {
		var dd1 = new dropDown($('.container-dropdown-header-social #box-dropdown'));

		$(document).click(function () {
			$('.container-dropdown-header-social .box-dropdown').removeClass('active');
		});
	});
	...
	// -------------------------- Dropdown --------------------------
	$('.container-dropdown-header-social .box-add-dropdown button').click(function (event) {
		let getDataActive = $('.container-dropdown-header-social .box-dropdown .title-active').text();
		const lengthList = $('.container-dropdown-header-social .box-dropdown ul li').length - 1;
		...
		var i;
		for (i = 0; i <= lengthList; ++i) {
			const dataList = $('.container-dropdown-header-social .box-dropdown ul li:eq(' + i + ')').attr('data');
			const dataStatus = $('.container-dropdown-header-social .box-dropdown ul li:eq(' + i + ')').attr('status');

			const labelActive = getDataActive.replace(/\s/g, '');
			if (labelActive) {
				$('.container-dropdown-header-social .box-dropdown ul li#' + labelActive).css('display', 'none');
				$('.container-dropdown-header-social .box-dropdown ul li#' + labelActive).attr('status', 'hidden');
			}
		...
		let label = $('.container-dropdown-header-social .box-dropdown .title-active').text();
		if ( 'x (twitter)' === label ) {
			label = 'twitter';
		}
		...
		// Box info all closed
		$('.sortable_repeater_header_social .repeater .box-info').removeClass('active');
		function solaceAppendRow2($element, defaultValueTitle = label, defaultValueContent = '', defaultValueLink = '', defaultValueSosmed = label) {
			let mycolor = $('li#customize-control-header_social_icon_color_'+label+'_setting .components-dropdown:eq(1) button');
		...
		event.preventDefault();
		solaceAppendRow2($('.control-sortable-repeater-header-social-add').parent());
		solaceGetAllInputs($('.control-sortable-repeater-header-social-add').parent());
	...
	$('.sortable_repeater_header_social').on('click', '.box-color-custom button', function() {
		let getInfoSosmed = $(this).parent().parent().parent().find('.box-input-sosmed .sosmed').val();

		// Trigger color
		if (getInfoSosmed === 'facebook') {
			let btn = $('li#customize-control-header_social_icon_color_facebook_setting .components-dropdown:eq(1) button');
			btn.trigger('click');
		} else if (getInfoSosmed === 'youtube') {
			let btn = $('li#customize-control-header_social_icon_color_youtube_setting .components-dropdown:eq(1) button');
			btn.trigger('click');
		} else if (getInfoSosmed === 'twitter') {
			let btn = $('li#customize-control-header_social_icon_color_twitter_setting .components-dropdown:eq(1) button');
			btn.trigger('click');
		} else if (getInfoSosmed === 'tiktok') {
			let btn = $('li#customize-control-header_social_icon_color_tiktok_setting .components-dropdown:eq(1) button');
			btn.trigger('click');
		} else if (getInfoSosmed === 'telegram') {
			let btn = $('li#customize-control-header_social_icon_color_telegram_setting .components-dropdown:eq(1) button');
			btn.trigger('click');
		} else if (getInfoSosmed === 'pinterest') {
			let btn = $('li#customize-control-header_social_icon_color_pinterest_setting .components-dropdown:eq(1) button');
			btn.trigger('click');
		} else if (getInfoSosmed === 'linkedin') {
			let btn = $('li#customize-control-header_social_icon_color_linkedin_setting .components-dropdown:eq(1) button');
			btn.trigger('click');
		}
	});
	...
	// Toggle save
	jQuery(window).on("load", function(){
		if ($('li#customize-control-header_social_toggle_icon input').is(":checked")) {
			// ON
			$('.sortable_repeater_header_social .repeater .box-info .box-color-custom').css('display', 'none');
			$('.sortable_repeater_header_social .repeater .box-info .box-color-ori').css('display', 'block');
		} else {
			// OFF
			$('.sortable_repeater_header_social .repeater .box-info .box-color-custom').css('display', 'block');
			$('.sortable_repeater_header_social .repeater .box-info .box-color-ori').css('display', 'none');
		}
	});

	// Toggle click
	$('li#customize-control-header_social_toggle_icon label').click(function(){
		if ($('li#customize-control-header_social_toggle_icon input').is(":checked")) {
			// OFF
			$('.sortable_repeater_header_social .repeater .box-info .box-color-custom').css('display', 'block');
			$('.sortable_repeater_header_social .repeater .box-info .box-color-ori').css('display', 'none');
