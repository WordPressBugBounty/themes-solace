...
use HFG\Core\Components\HeaderContact;

$text = parse_dynamic_tags(component_setting(HeaderContact::HEADER_CONTACT_ID));
$text = apply_filters('solace_translate_single_string', $text, HeaderContact::HEADER_CONTACT_ID);
if (!empty($text)) {
	$button_link = parse_dynamic_tags(component_setting(HeaderContact::LINK_ID));
	$button_link = apply_filters('solace_translate_single_string', $button_link, HeaderContact::LINK_ID);

	$contact = trim(get_theme_mod('header_contact_header_contact_setting', 'phonesolcommaemailsolcomma123456solcommacontact@example.comsolcommasolcommasolcommaphonesolcommaemail'));
	$data = explode("solcomma", $contact);

	echo '<div class="component-wrap component-wrap-header-contact">';
	$length = count($data) / 4;
